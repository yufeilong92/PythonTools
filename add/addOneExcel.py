...
class AddOneExecle(MainQuit):
    ...
    def showDialog(self, mainRoot):
        ...
        rowNum += 1
        Label(master=root, text="列One过滤特殊字符:", font=mFont, relief=mRlief).grid(row=rowNum, column=0,
                                                                                          sticky=mSticky,
                                                                                          pady=4)
        comboboxfiltratioeOne = ttk.Combobox(font=mFont, master=root, justify=tk.CENTER)
        comboboxfiltratioeOne.grid(row=rowNum, column=1, columnspan=2, sticky=mSticky, pady=4)
        comboboxfiltratioeOne['values'] = ["Yes", "No"]
        comboboxfiltratioeOne['state'] = "readonly"
        comboboxfiltratioeOne.config(font=mFont)
        comboboxfiltratioeOne.current(0)
        rowNum += 1
        Label(master=root, text="列Two过滤特殊字符:", font=mFont, relief=mRlief).grid(row=rowNum, column=0,
                                                                                          sticky=mSticky,
                                                                                          pady=4)
        comboboxfiltratioTwo = ttk.Combobox(font=mFont, master=root, justify=tk.CENTER)
        comboboxfiltratioTwo.grid(row=rowNum, column=1, columnspan=2, sticky=mSticky, pady=4)
        comboboxfiltratioTwo['values'] = ["Yes", "No"]
        comboboxfiltratioTwo['state'] = "readonly"
        comboboxfiltratioTwo.config(font=mFont)
        comboboxfiltratioTwo.current(1)
        rowNum += 1
        ...
        Label(master=root, text=".xlsx", font=mFont, relief=mRlief).grid(row=rowNum, column=2, sticky=mSticky,
                                                                         ...
                                                                         pady=4)
        rowNum += 1
        Label(master=root, text="保存列One:", font=mFont, relief=mRlief).grid(row=rowNum, column=0, sticky=mSticky,
                                                                              pady=4)
        ...
        Button(master=root, text="清空列One和Two", font=mFont, relief=mRlief,
               command=lambda: self.clearOneTwo(createNameOneEt, createNameTwoEt, showContext)).grid(row=rowNum,
                                                                                                     column=0,
                                                                                                     columnspan=3,
                                                                                                     sticky=mSticky,
                                                                                                     pady=4)
        ...
        Button(master=root, text="查询", font=mFont, relief=mRlief,
               command=lambda: self.queryList(selectExcelTV, createExcelFileTV, createNameEt, createNameOneEt,
                                              createNameTwoEt, showContext)).grid(row=rowNum, column=0, columnspan=3,
                                                                                  sticky=mSticky,
                                                                                  pady=4)
        rowNum += 1
        Button(master=root, text="添加", font=mFont, relief=mRlief,
               command=lambda: self.addSaveExcle(selectExcelTV, createExcelFileTV, createNameEt, createNameOneEt,
                                                 createNameTwoEt, showContext,comboboxfiltratioeOne,comboboxfiltratioTwo)).grid(row=rowNum, column=0, columnspan=3,
                                                                                     sticky=mSticky,
                                                                                     pady=4)
        rowNum += 1
        Button(master=root, text="粘贴One", font=mFont, relief=mRlief,
               command=lambda: self.postadd(selectExcelTV, createExcelFileTV, createNameEt, createNameOneEt,
                                            createNameTwoEt, showContext, comboboxfiltratioeOne)).grid(row=rowNum,
                                                                                                       column=0,
                                                                                                       sticky=mSticky,
                                                                                                       pady=4)
        Button(master=root, text="粘贴Two", font=mFont, relief=mRlief,
               command=lambda: self.postaddTwo(selectExcelTV, createExcelFileTV, createNameEt, createNameOneEt,
                                               createNameTwoEt, showContext, comboboxfiltratioTwo)).grid(row=rowNum,
                                                                                                         column=2,
                                                                                                         sticky=mSticky,
                                                                                                         pady=4)
        rowNum += 1
        Label(master=root, text="提示：默认优选添加到现有excele,默认表一,,其次是新建excele", font=mFont).grid(row=rowNum,
                                                                                                             column=0,
                                                                                                             columnspan=3,
                                                                                                             sticky=mSticky,
                                                                                                             pady=4)
    ...
    def addSaveExcle(self, selectExcelTV, createExcelFileTV, createNameEt, createNameOneEt, createNameTwoEt,
                     showContext,comboboxfiltratioeOne,comboboxfiltratioTwo):
        # 现有的数据
        ...
        try:
            ...
            if isrep:
                ...
                # =============保存=============
            if comboboxfiltratioeOne is not None:
                filtration = comboboxfiltratioeOne.get()
                if filtration=="Yes":
                    oneData = self.mattchData(oneData)
            if comboboxfiltratioTwo is not  None:
                filteration = comboboxfiltratioTwo.get()
                if filteration=="Yes":
    ...
    def postaddTwo(self, selectExcelTV, createExcelFileTV, createNameEt, createNameOneEt, createNameTwoEt, showContext,
        ...
        filtration = comboboxfiltratioTwo.get()
        if filtration == "Yes":
            context = self.mattchData(context)
        createNameTwoEt.insert(0, context)
        self.setTvContext(showContext, "", TypeBgColor.info)
        if createNameOneEt.get() == "":
            self.queryList(selectExcelTV, createExcelFileTV, createNameEt, createNameOneEt, createNameTwoEt,
                           showContext)
        else:
            self.addSaveExcle(selectExcelTV, createExcelFileTV, createNameEt, createNameOneEt, createNameTwoEt,
                              showContext,None,comboboxfiltratioTwo)
        pass
